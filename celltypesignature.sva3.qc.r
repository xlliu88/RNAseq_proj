...
## modified 2/28/2021
qc.path <- file.path(out.path, "qc2")
if(!file.exists(qc.path)) dir.create(qc.path)
logs <- file.path(out.path, "logs")
...
# cty.init(out.path)
  ...
  # eset.gc <- call.exprs(ab.raw, algorithm = mthd) # build exprimentSet data
  eset.gc <- gcrma(ab.raw)
  expr.gc <- exprs(eset.gc)                       # expression matrix
  
  sn <- sub("Mock.", "", colnames(expr.gc))
  ...
  ti <- sprintf("genecluster(top%d)_%s", ntop.hm, mthd)
...
# ==============================================================================
  ...
  svaobj <- sva(exprs(eset.gc), X) #, mod0)   # same result with or without mod0
...
# ################################################################################
  ...
  cty.genes <- getCellSpecificGenes2(eset.gc, svaX, queries = query.mat, 
                                     fc.thd = fc.thd, p.thd = p.thd,
                                     sva = T, svobj = svaobj,
                                     p2g = probe2gene,
                                     out.file = file.path(out.path, enrdGeneList.fn))
  
  ############################################################################
  ## read saved cell type enriched gene list
  cty.genes <- read_csv(file.path(out.path, enrdGeneList.fn), 
                        show_col_types = F) %>% 
    as.list() %>% 
...
wbc.up <- list(EIS = EIS.wbc,
  ...
  lapply(., function(x) x[x %in% gene_universe])
...
for(gt in c("WT", "clv")) {
    ...
    ggsave(file.path(out.sub, sprintf("fisherPlot_celltype.vs.%ssyn %s.pdf", gt, suffix)), 
           device = "pdf", width = 4.8, height = 5.5, units = "in", dpi = 300)
    
    #next
    ## go enrichment
    go.path <- file.path(out.sub, "GO_Syn&CtyEnrd-overlapped")
    ...
    for(r in 1:nrow(fisher)) {
      if(any(fisher[r, ] < fisher.p.thd)) {
        ...
        go.res <- list()
        go.res[["BP"]] <- goTair(x, "BP", outFormat = "all", out.path = go.path, fn.prefix = mk) #paste0(mk, "x"))
        go.res[["MF"]] <- goTair(x, "MF", outFormat = "all", out.path = go.path, fn.prefix = mk)
        go.res[["CC"]] <- goTair(x, "CC", outFormat = "all", out.path = go.path, fn.prefix = mk)
        
        for (nm in names(go.res)) {
          go.df <- go.res[[nm]]
          y <- colnames(go.df)[grepl("fisher", colnames(go.df), ignore.case = T)]
          goVisBar(go.res[[nm]],y = y, img.path = go.path, pthreshold = fisher.p.thd, 
